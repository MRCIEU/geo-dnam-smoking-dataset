...
geo.matrix.url <- function(gse) {
    file.path("ftp://ftp.ncbi.nlm.nih.gov/geo/series",
              paste(substring(gse, 1, 5), "nnn", sep=""),
              gse,
              "matrix",
...
readLinesUntil <- function(pattern, con, ...) {
    ...
    while (!finished) {
        ...
        lines <- c(lines, new)
        finished <- (length(grep("^!series_matrix_table_begin", new)) > 0
                     || length(new) < n)
        n <- 2*n
    }
    lines
...
geo.raw.url <- function(gse) {
    file.path("ftp://ftp.ncbi.nlm.nih.gov/geo/series",
              paste(substring(gse, 1,5), "nnn", sep=""),
              gse,
              "suppl",
              paste(gse, "RAW.tar", sep="_"))
}

recode <- function(x, coding) {
    y <- rep(NA, length(x))
    for (i in 1:length(coding)) 
        y[which(x == names(coding)[i])] <- coding[i]
    y
}
